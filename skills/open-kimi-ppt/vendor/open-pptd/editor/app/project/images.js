...
export function createImageStore(state) {
  ...
  async function preloadRemoteImages() {
    ...
    for (const page of state.deck.pages) {
      const backgroundSrc = page.background?.type === "image" ? page.background.src : null;
      if (backgroundSrc && !backgroundSrc.startsWith("data:") && !state.imageMap[backgroundSrc] && !seen.has(backgroundSrc)) {
        seen.add(backgroundSrc);
        todo.push(backgroundSrc);
      }
  ...
  async function preloadHandleImages(handle) {
    ...
    for (const page of state.deck.pages) {
      const sources = [];
      if (page.background?.type === "image" && page.background.src) sources.push(page.background.src);
      for (const el of page.elements || []) {
        if (el.elementType === "image" && el.src) sources.push(el.src);
      }
      for (const src of sources) {
        if (src.startsWith("data:") || state.imageMap[src] || seen.has(src)) continue;
        seen.add(src);
        const mime = extToMime(/\.([a-z0-9]+)$/i.exec(src)?.[1]);
        if (!mime) continue;
  ...
  function rebuildImageMap() {
    ...
    for (const page of state.deck.pages) {
      const backgroundSrc = page.background?.type === "image" ? page.background.src : null;
      if (backgroundSrc) {
        if (backgroundSrc.startsWith("data:")) next[backgroundSrc] = backgroundSrc;
        else if (state.imageMap[backgroundSrc]) next[backgroundSrc] = state.imageMap[backgroundSrc];
      }
      for (const el of page.elements || []) {
  ...
  function persistDataUrlImages() {
    const out = [];
    const seen = new Set();
    for (const [pageIndex, page] of state.deck.pages.entries()) {
      const refs = [];
      if (page.background?.type === "image" && page.background.src) {
        refs.push({ holder: page.background, key: "src", id: `background-${pageIndex + 1}` });
      }
      for (const el of page.elements || []) {
        if (el.elementType === "image" && el.src) refs.push({ holder: el, key: "src", id: el.elementId });
      }
      for (const ref of refs) {
        const src = ref.holder[ref.key];
        const dataUrl = src.startsWith("data:")
          ? src
          : String(state.imageMap[src] || "").startsWith("data:")
            ? state.imageMap[src]
            : null;
        ...
        if (src.startsWith("data:")) {
          const rel = `media/${ref.id}.${info.ext}`;
          state.imageMap[rel] = dataUrl; // 新路径 → 原 dataURL，预览保持可用
          ref.holder[ref.key] = rel;
          out.push({ path: rel, b64: bytesToBase64(info.bytes) });
        } else {
          out.push({ path: src, b64: bytesToBase64(info.bytes) });
        }
      }
    }
    return out;
...
export function mediaFilesOfDeck(deckSnapshot, imageMap = {}) {
  const files = [];
  const seen = new Set();
  for (const [pageIndex, page] of (deckSnapshot.pages || []).entries()) {
    const refs = [];
    if (page.background?.type === "image" && page.background.src) {
      refs.push({ holder: page.background, key: "src", id: `background-${pageIndex + 1}` });
    }
    ...
    for (const ref of refs) {
      ...
      if (src.startsWith("data:")) {
        const rel = `media/${ref.id}.${decoded.ext}`;
        files.push({ path: rel, b64: bytesToBase64(decoded.bytes) });
        ref.holder[ref.key] = rel;
      } else {
        files.push({ path: src, b64: bytesToBase64(decoded.bytes) });
      }
    }
  }
  return files;
